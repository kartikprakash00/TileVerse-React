...
const Game2187 = () => {
    ...
    const updateHighScore = async (score) => {
        try {
            ...
            if (token) {
                const response = await axios.post(backendUrl + '/api/user/highscore2187', { userId, score }, { headers: { token } })

                if (response.data.success) {
                    console.log(response.data.highscore2187)
                } else {
    ...
    return (
        <div className='game'>
            <h1 className='game-2187'>2187</h1>
            <hr />
